...
| `POST`/`GET` | `/api/v1/funding-arb/watches` | Follow the scan (omit `symbol`) or one pair; notify when after-fee net ≥ `minProfit` |
| `GET`/`PATCH`/`DELETE` | `/api/v1/funding-arb/watches/{id}` | Get / edit settings / delete a funding-arb follow |
| `POST` | `/api/v1/funding-arb/watches/{id}/pause` | Pause a follow without deleting it |
| `POST` | `/api/v1/funding-arb/watches/{id}/resume` | Resume a paused follow |
...
| `POST` | `/api/v1/scanner/rules` | Create scanner rule (RSI / MA / volume, combined with matchMode all\|any) |
...
**Cross-exchange price diff:** watches (`/api/v1/price-diff/watches`) take a `notional` and `minProfit`. The worker walks live books and opens an opportunity only when that size can be bought and sold and after-fee profit (including slippage) meets the floor — not from ticker last-price gaps. Open state is durable; no duplicate while open; re-opens after the fill drops and returns. Missing books skip that venue. Interval `PRICE_DIFF_CHECK_INTERVAL` (default `30s`). **Executable quote** (`/price-diff/quote` or `/opportunities/{id}/quote`) walks the buy asks and sell bids for a `notional` or `quantity` and returns average prices, slippage, profit after fees, usable money, and max still-profitable size. **Scan** (`/price-diff/quote/scan` or `/watches/{id}/quote`) ranks every venue pair at that size.
