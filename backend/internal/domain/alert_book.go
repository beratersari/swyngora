...
func NormalizeAlertKind(s string) (AlertKind, bool) {
	...
	switch AlertKind(s) {
	case AlertKindPrice, AlertKindImbalance, AlertKindWall, AlertKindLiqFeed, AlertKindLiqCascade:
...
func ValidateAlertSpec(kind AlertKind, condition string, target, rangePct float64) error {
	...
	if !ok {
		return fmt.Errorf("%w: kind must be price, imbalance, wall, liquidation_feed, or liquidation_cascade", ErrInvalidArgument)
	}
	...
	case AlertKindLiqFeed, AlertKindLiqCascade:
		return validateLiqAlertSpec(kind, cond, target)
	}
	return nil
}
