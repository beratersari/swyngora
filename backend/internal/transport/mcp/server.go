...
func registerTools(s *server.MCPServer, api DataPort, accounts *account.Service) {
	...
	addTool(mcp.NewTool("get_liquidation_heatmap",
		mcp.WithDescription("CoinGlass-style price × time liquidation intensity map from historical open interest, each venue's own price (never borrowed from the other exchange), the hunt leverage mix, and observed liquidation prints. range=12h|24h|3d|7d. Returns Binance, Bybit, and combined (sum) grids plus a review of whether later price hit the hot zones in 1h/4h/12h (hit rate, false signals, time-to-hit, liquidation increase). Not financial advice."),
		mcp.WithString("symbol", mcp.Required(), mcp.Description("Pair e.g. BTCUSDT")),
