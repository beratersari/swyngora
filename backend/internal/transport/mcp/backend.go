...
func (b *Backend) GetAroundSimilar(ctx context.Context, exchange, symbol, lookback, interval, direction string, minReturnPct float64, limit int, window, during, fields string) (json.RawMessage, error) {
	if b.Market == nil {
		return nil, fmt.Errorf("%w: market not configured", domain.ErrUpstream)
	}
	got, err := b.Market.GetAroundSimilar(ctx, exchange, symbol, lookback, interval, direction, minReturnPct, limit, window, during, fields)
	if err != nil {
		return nil, err
	}
	return mustJSON(got)
