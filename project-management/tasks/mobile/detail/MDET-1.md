# MDET-1: RTK detail endpoints (intervals, ticker, supply, candles, indicators)

| Field | Value |
|---|---|
| **ID** | MDET-1 |
| **Epic** | mobile-coin-detail |
| **Status** | done |
| **Area** | mobile |
| **Path** | `project-management/tasks/mobile/detail/MDET-1.md` |


## Summary

Extend mobile `libs/api/endpoints/marketApi.ts` with listIntervals, getTicker24h, getSupply, getCandles, getIndicators using OpenAPI-generated types. Wire tags in baseApi. No page UI.

## Design

`docs/design/mobile-coin-detail.md`  
Web field specs: `tasks/frontend/detail/DET-A.md`, `DET-B.md`

## Acceptance

- Matches design section for this task  
- UI only via `src/components/` Atomic layers (no `modules/*/components`)  
- Tests where logic is non-trivial  
- Update status here + `board.md` when done  

## Status

Update status in this file and in `project-management/board.md` when work starts/finishes.
