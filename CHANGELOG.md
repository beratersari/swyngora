...
- **Funding arb watches:** follow a pair and notify via the alert webhook when after-fee horizon net is at least `minProfit`. Same long/short while still above the floor does not re-notify; a direction flip closes the old signal and opens a new one (`POST/GET/DELETE /api/v1/funding-arb/watches`, `GET /api/v1/funding-arb/signals`, MCP `create_funding_arb_watch` / `list_funding_arb_signals`) (`docs/features/funding-arb.md`)
