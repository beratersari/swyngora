...
### Added
- **Liquidation hunt cascade paths:** each venue lists zones from last price outward (distance, estimated liq in the zone, hop cost) and whether earlier estimated liquidations cheapen the next hop. Zone and desk P&L stay the same. Web Hunt keeps Compare as the default and puts the path on a sub-tab (`/liquidations?view=hunt&panel=path`) (`GET /api/v1/market/liquidation-hunt`, MCP `estimate_liquidation_hunt`) (`docs/features/liquidation-hunt.md`)
