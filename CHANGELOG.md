...
### Added
- **Liquidation hunt heatmap:** CoinGlass-style price × time intensity from historical OI, **each venue's own price** (no Binance↔Bybit fallback), the hunt leverage mix, and observed liquidations. Ranges 12h / 24h / 3d / 7d. Binance and Bybit separately plus combined sum. `review` scores 1h / 4h / 12h hot zones and reports how many signals have enough price and liquidation history to validate; hit rate and liq-increase use only that validated set (`GET /api/v1/market/liquidation-hunt/heatmap`, MCP `get_liquidation_heatmap`). Coin detail Tape tab (`docs/features/liquidation-hunt.md`)
