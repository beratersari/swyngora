...
### Added
- **Liquidation hunt custom score weights:** optional `weightProximity` / `weightBook` / `weightEfficiency` / `weightTrend` / `weightCrowding` / `weightFlow` (must sum to 100). Disabled factors are omitted. Missing data is not substituted. `scoreMix` shows requested vs used weights. `scoreCompare` is default vs applied (up/down scores, lean, per-factor effects) for the desk and for AI. Web Hunt previews default vs draft scores in the Custom weights dialog (`GET /api/v1/market/liquidation-hunt`, MCP `estimate_liquidation_hunt`) (`docs/features/liquidation-hunt.md`)
