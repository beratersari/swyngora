...
### Added
- **Liquidation cascade:** detect a short burst of long or short liquidations far above that stream's typical rate (1m / 5m / 15m vs the prior 6 hours). Binance and Bybit stay separate; `both.agree` only when the same side is cascading on both. Coin view plus market-wide scan (`GET /api/v1/market/liquidation-cascade`, `.../scan`, MCP `get_liquidation_cascade` / `scan_liquidation_cascades`, web `/liquidations?view=cascade`) (`docs/features/liquidations.md`)
