...
- **Funding arb watches:** omit `symbol` to follow the funding-arb scan and notify when a new coin's after-fee net is at least `minProfit`. Same coin + same long/short while still above the floor does not re-notify; a drop then re-cross notifies again (`POST/GET/DELETE /api/v1/funding-arb/watches`, `GET /api/v1/funding-arb/signals`, MCP `create_funding_arb_watch` / `list_funding_arb_signals`) (`docs/features/funding-arb.md`)
