...
- **Funding arb watches:** omit `symbol` to follow the funding-arb scan and notify when a new coin's after-fee net is at least `minProfit`. Same coin + same long/short while still above the floor does not re-notify; a drop then re-cross notifies again. A coin missing from the scan is re-quoted and closed only when net is really below the floor. Pause/resume and PATCH settings (`POST .../pause`, `POST .../resume`, `PATCH /api/v1/funding-arb/watches/{id}`, MCP `pause_funding_arb_watch` / `update_funding_arb_watch`) (`docs/features/funding-arb.md`)
