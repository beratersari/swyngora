...
- **Price-diff watch edit / pause:** `PATCH /api/v1/price-diff/watches/{id}` changes notional, minProfit, minDurationSec, and fees (duration timer resets). Pause stops searching and closes open opportunities; resume starts the duration wait from zero (`POST .../pause`, `POST .../resume`, MCP `update_price_diff_watch` / `pause_price_diff_watch` / `resume_price_diff_watch`) (`docs/features/price-diff.md`)
