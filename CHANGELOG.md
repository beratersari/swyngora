...
### Added
- **Liquidation max pain:** largest estimated liquidation pocket above last (shorts) and below last (longs) — price, size, distance, side. Separate from the hunt target. Web `/liquidations?view=max-pain`, `GET /api/v1/market/liquidation-max-pain`, MCP `get_liquidation_max_pain` (`docs/features/liquidation-max-pain.md`)
- **Liquidation hunt custom score weights:** optional `weightProximity` / `weightBook` / `weightEfficiency` / `weightTrend` / `weightCrowding` / `weightFlow` (must sum to 100). Disabled factors are omitted. Missing data is not substituted. `scoreMix` shows requested vs used weights. `scoreCompare` is default vs applied with separate `upFactors` / `downFactors` and `upLargestChange` / `downLargestChange` (the factor that moved that direction the most). Web Hunt previews both sides in the Custom weights dialog (`GET /api/v1/market/liquidation-hunt`, MCP `estimate_liquidation_hunt`) (`docs/features/liquidation-hunt.md`)
- **Liquidation hunt cascade paths:** each venue lists zones from last price outward (distance, estimated liq in the zone, hop cost) and whether earlier estimated liquidations cheapen the next hop. Each hop has a feed `role` / `strength` (omitted when the book or zone size is missing) and the path marks where self-fueling stalls. Zone and desk P&L stay the same. Web Hunt keeps Compare as the default and puts the path on a sub-tab (`/liquidations?view=hunt&panel=path`) (`GET /api/v1/market/liquidation-hunt`, MCP `estimate_liquidation_hunt`) (`docs/features/liquidation-hunt.md`)
- **Liquidation hunt direction scores:** each venue now ranks up vs down (0–100 plus reasons) from zone distance, visible book cost, price+OI trend, crowding/funding, and recent taker/liquidation flow. Zone and desk P&L numbers stay the same. Combined `bias` is OI-weighted. Web `/liquidations?view=hunt` compares both sides (`GET /api/v1/market/liquidation-hunt`, MCP `estimate_liquidation_hunt`) (`docs/features/liquidation-hunt.md`)
...
- **Liquidation reconnect fill:** when Binance or Bybit drops and comes back, that venue's own history is used to fill the hole. Live and history prints that are the same event count once. A fully filled hole is removed from `feed.gaps`; a partial fill leaves the leftover and `missingSeconds` shows how much is still missing. Venues stay separate (`docs/features/liquidations.md`)
- **Liquidation cascade:** detect a short burst of long or short liquidations far above that stream's typical rate (1m / 5m / 15m vs the prior 6 hours). Binance and Bybit stay separate; `both.agree` only when the same side is cascading on both. `episodes` lists each wave in the last 24h (start, duration, long/short notional, price move); overlapping same-side waves become one combined event. Coin view plus market-wide scan (`GET /api/v1/market/liquidation-cascade`, `.../scan`, MCP `get_liquidation_cascade` / `scan_liquidation_cascades`, web `/liquidations?view=cascade`) (`docs/features/liquidations.md`)
...
- **Price-diff watch edit / pause:** `PATCH /api/v1/price-diff/watches/{id}` changes notional, minProfit, minDurationSec, fees, and `exchanges` (duration timer resets). Pause marks the watch paused first so an in-flight tick cannot open; open opportunities close; resume starts the duration wait from zero (`POST .../pause`, `POST .../resume`, MCP `update_price_diff_watch` / `pause_price_diff_watch` / `resume_price_diff_watch`) (`docs/features/price-diff.md`)
...
- **Holders cache key:** `WBTC` / `STETH` / `WSTETH` no longer share the 1h holders cache with `W` / `ST` / `WST`. Pair forms still normalize (`ETHBTC` → ETH, `WBTCUSDT` → WBTC) (`docs/features/holders.md`)
