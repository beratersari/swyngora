...
### Added
- **Liquidation hunt custom score weights:** optional `weightProximity` / `weightBook` / `weightEfficiency` / `weightTrend` / `weightCrowding` / `weightFlow` (must sum to 100). Disabled factors are omitted. Missing data is not substituted. `scoreMix` shows requested vs used weights. Web Hunt has a Custom weights dialog and reset (`GET /api/v1/market/liquidation-hunt`, MCP `estimate_liquidation_hunt`) (`docs/features/liquidation-hunt.md`)
