...
### Added
- **Liquidation alerts:** repeating `liquidation_feed` (venue down/stalled longer than N seconds) and `liquidation_cascade` (min grade on a coin or `symbol=all`). Same edge machine as book alerts — one fire, quiet while still true, re-arm when the feed is live or the cascade ends. Cascade webhook names the exchange and coin (`POST /api/v1/alerts`, MCP `create_liquidation_feed_alert` / `create_liquidation_cascade_alert`) (`docs/features/price-alerts.md`)
