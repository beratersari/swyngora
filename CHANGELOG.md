...
### Added
- **Market-wide order book:** combined Binance + Coinbase + Bybit live depth; totals use only the band every venue can reach (requested ±% when all cover it) (`GET /api/v1/market/orderbook/combined`, MCP `analyze_market_orderbook`) (`docs/features/order-book.md`)
