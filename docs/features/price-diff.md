...
| MCP | `create_price_diff_watch`, `list_price_diff_watches`, `get_price_diff_watch`, `update_price_diff_watch`, `pause_price_diff_watch`, `resume_price_diff_watch`, `delete_price_diff_watch`, `list_price_diff_opportunities`, `get_price_diff_opportunity`, `quote_price_diff`, `quote_price_diff_opportunity`, `scan_price_diff_quotes`, `quote_price_diff_watch` |
...
```bash
cd backend
go test ./internal/domain/ ./internal/service/pricediff/ ./internal/transport/http/handler/ -run 'PriceDiff|NetDiff|Fresh|Opportunity|Skip|Persist|Quote|Pause|Resume|UpdateWatch' -count=1
```
