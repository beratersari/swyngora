...
`/heatmap?view=rsi` is a **ranked RSI scatter** (one timeframe, one dot per pair) from `GET /api/v1/market/rsi-heatmap`. See [`rsi-heatmap.md`](rsi-heatmap.md).
...
- `GET /api/v1/market/liquidation-hunt` hypothetical per-venue hunt (spot size to reach estimated liq zones + desk result + up/down ease scores). See [`liquidation-hunt.md`](liquidation-hunt.md).
...
- `GET /api/v1/market/around/similar` past important-move setups like the current tape; unique events; matching-bar after horizons with similarity bands. See [`around.md`](around.md).
