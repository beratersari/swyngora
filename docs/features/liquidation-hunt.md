...
`scoreMix` lists `requested`, `used`, `missing`, and `disabled`. `scoreCompare` is default vs applied: up/down scores, lean, and **separate** `upFactors` / `downFactors` (`defaultPct` / `appliedPct` / `defaultEffect` / `appliedEffect` / `deltaEffect`). Combined rows list `usedVenues` / `missingVenues` / `venues[]` so a used status is not paired with another venue's missing detail. Combined numbers are OI-weighted across **used** venues only. `delta.upLargestChange` / `downLargestChange` is the factor whose custom mix moved that direction the most (`deltaEffect` and a one-line `summary`). Use that to compare mixes (desk or AI) without re-deriving the math. A missing input is not replaced (no 24h ticker for trend, no 4h window for 1h flow). Web: **Custom weights** on Hunt previews default vs draft scores, both directions, before apply.
