...
1. **Create** a plan: coin (`exchange` + `symbol`), cash `amount`, optional **`name`**, `frequency`, optional `startAt`, optional `timeZone` / `hour` / `minute`, optional `maxPrice`, optional `budget`, optional `endDate` / `endsAt`.
2. **Schedules**
...
3. **Local clock:** IANA `timeZone` (e.g. `Europe/Istanbul`, `America/New_York`). Empty timezone keeps the previous UTC `startAt` clock. `hour` `0–23` and `minute` `0–59` pin the **wall clock** in that zone. Weekday and month day are local. **DST:** a missing hour on a spring-forward day snaps to the first valid local time after the gap; a repeated hour on a fall-back day uses the first occurrence. Interval plans stay elapsed UTC hours.
...
5. **Pause / resume / delete** at any time. **PATCH** updates name, amount, schedule, timezone/clock, `maxPrice`, `budget`, and/or end.
...
7. **`maxPrice`:** optional cap (0 = none). The run is skipped when **last**, **slipped fill**, or **fee-inclusive unit cost** would exceed it. Example: cap 65000 — last 66000 does not buy; last 64000 buys at the current slipped market; last 64980 can still skip if slip+fee pushes the real unit over 65000. A PATCH to `maxPrice` (or pause / budget / end) takes the same book lock as the fill; the live plan is re-read **inside that lock** immediately before the cash debit, so an in-flight worker cannot complete with the old limit. A skipped run is **failed** with that reason; the plan stays **active** and advances.
8. **`budget`:** optional total cash cap (0 = none). `spent` is the **wallet debit** (slipped fill + taker fee), not raw order notional. Each run spends `min(amount, remaining)` of that cash. The last slice may be smaller than `amount`. When leftover cannot cover a minimum buy after fee and slippage, the run fails with `budget exhausted`. A successful fill writes the trade, `spent`, and the succeeded run **in one store transaction**, so a crash after the buy cannot leave budget uncounted. The same period is never charged twice (`last_period_key` + unique period).
9. **Retries:** temporary failures (`market price unavailable`, `order failed`, `insufficient cash`, `plan unavailable`) leave the period due and retry on the next worker tick. They do not advance the schedule and do not add to `spent`. A retry uses a stable idempotency key for that plan period so a recovered fill is not bought or billed again. Permanent skips (`maxPrice`, budget exhausted, paused, ended) still fail the period and move on.
10. **`endDate` / `endsAt`:** optional last inclusive run. `endDate` is `YYYY-MM-DD` in the plan timezone (that local day still runs). `endsAt` is an RFC3339 instant. After the last allowed slot the plan status is **`ended`**. Resume requires extending the end first.
11. **Idempotency:** `UNIQUE(plan_id, period_key)` on run rows — only one succeeded fill per period. Temporary `in_progress` / retryable rows are updated in place.
12. **Catch-up:** if the server was down across multiple periods, only the **latest** due slot runs once (intermediate slots are skipped).
...
| `PATCH` | `/api/v1/portfolio/recurring-buys/{id}` | Update name / amount / schedule / timeZone / clock / maxPrice / budget / endDate |
...
- Weekday / day-of-month / clock use **UTC** unless `timeZone` is set (then DST rules of that zone apply).
