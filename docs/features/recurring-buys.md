...
8. **`budget`:** optional total cash cap (0 = none). `spent` is the **wallet debit** (slipped fill + taker fee), not raw order notional. Each run spends `min(amount, remaining)` of that cash. The last slice may be smaller than `amount`. When leftover cannot cover a minimum buy after fee and slippage, the run fails with `budget exhausted`.
9. **`endDate` / `endsAt`:** optional last inclusive run. `endDate` is `YYYY-MM-DD` in the plan timezone (that local day still runs). `endsAt` is an RFC3339 instant. After the last allowed slot the plan status is **`ended`**. Resume requires extending the end first.
10. **Insufficient cash** (or market unavailable): that run is stored as **failed**; the plan stays **active** and advances to the next period.
11. **Idempotency:** `UNIQUE(plan_id, period_key)` on run rows — only one claim per period.
