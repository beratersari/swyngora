...
- Each `create_agent` model step retries the current provider (1 try + 3 retries, exponential backoff), then calls the other of ChatXAI / ChatOllama if that client can be built. Next step starts on the primary again. The CLI spinner / Process stream show `Retrying {model} (2/4)…`, `failed after 4 tries`, and `Falling back to {model}…`. If both fail, the CLI/serve error lists primary then fallback (model + exception). `analyze()` is still a bare `invoke` (no retry or fallback).
...
- HTTP/Telegram pass `clientId`; Python tools bind that id (and send `SWYNGORA_API_TOKEN` / `X-Client-Id`) so the model cannot switch tenants. Auth uses this task’s ContextVars only — a worker without a bind fails closed (no guessing `stack[-1]`). Desk gather copies those ContextVars onto pool workers via `submit_with_bound_context`. Progress emit recovers a callback only when exactly one chat is in flight. Missing `canTrade` / `canManageKeys` on the Python HTTP API default to deny. Reserved ids (`ai-assistant`, `http-default`, `anonymous`) are rejected by the backend.
...
Recurring buys: `create_recurring_buy`, `update_recurring_buy`, `list_recurring_buys`, `get_recurring_buy`, `pause_recurring_buy`, `resume_recurring_buy`, `delete_recurring_buy`, `list_recurring_buy_runs` (`timeZone` / `hour` / `minute` DST-aware local clock; `maxPrice` skips if last or fee+slippage unit would exceed it; `budget` total cash cap; `endDate` / `endsAt` last inclusive run). See `docs/features/recurring-buys.md`.  
...
Paper margin: `place_margin_order`, `set_margin_mode`, `adjust_margin`, `repay_margin_debt`, `list_margin_positions`, `close_margin_position`, `set_margin_brackets`, `list_margin_orders`, `cancel_margin_order`, `list_margin_trades` (`place_margin_order` / `close_margin_position` accept `idempotencyKey`). See `docs/features/paper-margin.md`.  
Scanner backtests: `start_scanner_backtest`, `list_scanner_backtests`, `get_scanner_backtest`, `cancel_scanner_backtest`, `list_scanner_backtest_signals`. See `docs/features/indicator-scanner.md`.  
Price-diff: `create_price_diff_watch`, `list_price_diff_watches`, `get_price_diff_watch`, `update_price_diff_watch`, `pause_price_diff_watch`, `resume_price_diff_watch`, `delete_price_diff_watch`, `list_price_diff_opportunities`, `get_price_diff_opportunity`, `quote_price_diff`, `quote_price_diff_opportunity`, `scan_price_diff_quotes`, `quote_price_diff_watch` (walk books for a size or compare every venue pair: average buy/sell, slippage, profit after fees, usable money, max size). See `docs/features/price-diff.md`.
