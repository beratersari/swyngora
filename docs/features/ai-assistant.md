...
Impact: `estimate_market_impact` walks live depth for a market buy/sell (`quantity` or `notional`). Walls include `behavior` (`short` / `persistent` / `suspicious`). See `docs/features/order-book.md`.  
