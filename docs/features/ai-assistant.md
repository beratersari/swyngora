...
Impact: `estimate_market_impact` walks live depth for a market buy/sell (`quantity` or `notional`). Liquidity: `get_market_liquidity` (0–100 + weaker side, per venue + market-wide). Walls include `behavior` (`short` / `persistent` / `suspicious`). See `docs/features/order-book.md`.  
